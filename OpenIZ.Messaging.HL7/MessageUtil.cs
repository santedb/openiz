...
namespace OpenIZ.Messaging.HL7
{
    ...
    public class MessageUtil
    {
        ...
        public static IEnumerable<EntityAddress> ConvertAddresses(XAD[] addresses)
        {
            ...
            for (int i = 0; i < addresses.Length; i++)
            {
                ...
                if (!string.IsNullOrEmpty(addresses[i].AddressType.Value) && !string.IsNullOrWhiteSpace(addresses[i].AddressType.Value))
                {
                    ...
                    if (concept == null)
                    {
                        // HACK: Ignore unknown address types
                        continue;
        ...
        public static EntityIdentifier ConvertIdentifier(CX identifier, string aaHint)
        {
            return MessageUtil.ConvertIdentifiers(new CX[] { identifier }, aaHint).FirstOrDefault();
        }
        ...
        public static IEnumerable<EntityIdentifier> ConvertIdentifiers(CX[] identifiers, String aaHint)
        {
            ...
            for (var i = 0; i < identifiers.Length; i++)
            {
                ...
                // AA was not found, attempt to find an AA with code that matches the aaHint
                if (assigningAuthority == null && !String.IsNullOrEmpty(aaHint)) // HACK: Look up by name of assigning device, this should be changed to lookup by device and facility
                    assigningAuthority = assigningAuthorityService.Find(a => a.DomainName == aaHint).FirstOrDefault();
        ...
        public static EntityTelecomAddress ConvertXTN(XTN xtn, String defaultUse)
        {
            ...
            if (string.IsNullOrEmpty(xtn.TelecommunicationUseCode.Value))
                xtn.TelecommunicationUseCode.Value = defaultUse;

            var concept = GetConcept(xtn.TelecommunicationUseCode.Value, "urn:oid:1.3.6.1.4.1.33349.3.1.5.9.3.52");

            // TODO: cleanup
            if (concept == null)
            {
                throw new ArgumentException($"Code not known {xtn.TelecommunicationUseCode.Value}");
            }
        ...
        public static IMessage CreateNack(IMessage request, List<IResultDetail> errors, Type errType)
        {
            ...
            foreach (var dtl in errors)
            {
                try
                {
                    ...
                    {
                        var tErr = UpdateERR(errSeg as NHapi.Model.V25.Segment.ERR, dtl);
        ...
        public static Patient CreatePatient(MSH msh, EVN evn, PID pid, PD1 pd1, List<IResultDetail> details)
        {
            ...
            patient.Identifiers.AddRange(MessageUtil.ConvertIdentifiers(pid.GetPatientIdentifierList(), String.Empty));
            patient.Identifiers.AddRange(MessageUtil.ConvertIdentifiers(pid.GetAlternatePatientIDPID(), String.Empty));

            if(!String.IsNullOrEmpty(pid.PatientID.ID.Value))
                patient.Identifiers.Add(MessageUtil.ConvertIdentifier(pid.PatientID, msh.SendingApplication.NamespaceID.Value));
            patient.Identifiers = patient.Identifiers.Where(o => !String.IsNullOrEmpty(o.Value) && o.AuthorityKey.HasValue).ToList();
            ...
            patient.Telecoms.AddRange(pid.GetPhoneNumberHome().Select(o => ConvertXTN(o, "PRN")));
            patient.Telecoms.AddRange(pid.GetPhoneNumberBusiness().Select(o => ConvertXTN(o, "WPN")));

            if (pid.MotherSIdentifierRepetitionsUsed > 0 || pid.MotherSMaidenNameRepetitionsUsed > 0)
            {
                var person = new Person
                {
                    Identifiers = ConvertIdentifiers(pid.GetMotherSIdentifier(), msh.SendingApplication.NamespaceID.Value).ToList(),
            ...
            // FIX: When registering a patient that already exists we should ensure that the patient doesn't already exist
            var uniqueIds = patient.Identifiers.Where(o => o.LoadProperty<AssigningAuthority>(nameof(EntityIdentifier.Authority)).IsUnique);
            var patientService = ApplicationContext.Current.GetService<IRepositoryService<Patient>>();
            foreach(var uid in uniqueIds)
            {
                var existingPatient = patientService.Find(p => p.Identifiers.Any(i => i.AuthorityKey == uid.AuthorityKey && i.Value == uid.Value)).FirstOrDefault();
                if(existingPatient != null)
                {
                    patient.Key = existingPatient.Key;
                    break;
                }
            }
            return patient;
